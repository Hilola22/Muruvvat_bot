      ...
      if (!user) {
        ...
        if (user.last_state === "location") {
          if ("location" in ctx.message!) {
            ...
            const oldmsg = await ctx.reply("...", Markup.removeKeyboard());
            ...
            ctx.reply("Bo'limlardan birini tanlang: ", {
              reply_markup: {
                inline_keyboard: [
                  [
                    {
                      text: "Murojat yo'llash",
                      callback_data: `murojat_yollash__${user_id}`,
                    },
                  ],
                ],
              },
      ...
      if (!user) {
        ...
        if (user.last_state == "photo") {
          if (ctx.message && "photo" in ctx.message) {
            ...
            const newImage = await UserImg.create({
              user_id: user_id!,
              img_url: uniqueFileName,
            });
            const readStream = createReadStream(filePath);
            await ctx.replyWithPhoto({ source: readStream });

            user.last_state = "done";
            await user.save();
          }
        }
      }
  ...
  async onClickMurojatYollash(ctx: Context) {
    try {
      ...
      if (!user) {
        ...
        ctx.reply("Iltimos Rasmingizni yuboring: ", Markup.removeKeyboard());
      }
    } catch (error) {
